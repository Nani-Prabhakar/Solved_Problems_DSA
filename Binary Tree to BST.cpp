...
class Solution{
  public:
    ...
    // Function to Construct BST from In-Order Traversal
    void constructBST(Node* root, vector<int> &inorder, int &index)
    {
        if (!root)
            return;
    ...
    Node *binaryTreeToBST(Node *root)
    {
        ...
        // Sort the inorder vector
        sort(inorder.begin(), inorder.end());

        // Construct BST from In-Order
        int index = 0;
